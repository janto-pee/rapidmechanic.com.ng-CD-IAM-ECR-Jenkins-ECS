...
export async function createUserHandler(
  ...
  const body = req.body;
  try {
    const userEmail = await findUserByEmail(body.email);
    if (userEmail) {
      return res.status(400).json({ error: "email is already registered" });
    }
    const userUsername = await findUserByEmail(body.username);
    if (userUsername) {
